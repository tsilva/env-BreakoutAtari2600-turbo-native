...
The drift guard scans every tracked UTF-8, NUL-free current contract, code, workflow, lock, and release-skill file. Its content rules allow only explicit removal or negation, upstream/legal provenance, literal stakeholder/domain language that does not establish authority, and actual versioned changelog/benchmark history. The sole file exclusion is the guard's own test module because it embeds the forbidden adversarial fixtures.
